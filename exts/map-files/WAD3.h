...
public:
	...
	void PrintError()
	{
		if (m_dwError)
		{
			...
			std::cout << "Whoops, something went wrong.  GetLastError() = " << m_dwError << std::endl << (LPTSTR)lpMsgBuf << std::endl;
		...
		if (m_szErrorMessage)
		{
			std::cout << "Whoops, something went wrong. " << std::endl << m_szErrorMessage << std::endl;
		}
	}
};
